  ...
  try{
    if(navigator.share){
      await navigator.share({
        title:"Beat Apurado Radio",
        text:(window.BeatI18n?.t("shareText")||"🎧 Escuta agora a Beat Apurado Radio! 🚀"),
        url:RADIO_URL
      });
    }else{
      await navigator.clipboard.writeText(RADIO_URL);
      alert(window.BeatI18n?.t("copied")||"Link da Beat Apurado copiado! 🚀");
    }
  }catch(e){}
});

/* PWA — botão no topo + ocultar quando já estiver instalada */
...
function updateInstallButton(){
  if(!installTopBtn) return;

  if(isPWA()){
    installTopBtn.style.display="none";
  }else{
    installTopBtn.style.display="";
  }
}
...
installTopBtn?.addEventListener("click",async()=>{
  ...
  if(ios){
    ...
    alert(
      window.BeatI18n?.t("installUnavailable") ||
      "A instalação ainda não está disponível neste momento."
    );

  }
});

/* verifica assim que o JS carrega */
updateInstallButton();

/* confere novamente quando a página termina de carregar */
...
/* detecta mudança para modo PWA */
window.matchMedia("(display-mode: standalone)")
  .addEventListener?.("change",updateInstallButton);


/* PIX/BMC */
const pixBox=document.getElementById("pix-float"),
      bmcBox=document.getElementById("bmc-float");

document.getElementById("pixClose")?.addEventListener("click",()=>{
  pixBox.style.display="none";
});

document.getElementById("bmcClose")?.addEventListener("click",()=>{
  bmcBox.style.display="none";
});
...
function savedLang(){
  try{
    return window.BeatI18n?.normalizeLang(
...
async function chooseLanguage(lang){
  ...
  const safe=
    window.BeatI18n?.normalizeLang(lang) ||
    "pt-BR";
...
window.addEventListener(
  ...
  e=>setActiveLanguageButton(e.detail?.lang)
);
...
async function applyCountry(countryCode,source="real"){
  ...
  const lang=
    ...
    (
      isCountryTest
        ?window.BeatI18n?.languageForCountry(country)
        :savedLang()
    ) ||
    window.BeatI18n?.languageForCountry(country) ||
    "en";
...
async function syncRegion(){
  ...
  if(testCountry){
    await applyCountry(testCountry,"test");
    return;
  }
  ...
  try{
    ...
    console.warn(
      "[Beat Apurado] Falha ao detectar país; usando Brasil/Português como fallback.",
      e
    );
  }
}
